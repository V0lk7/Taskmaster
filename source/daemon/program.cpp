...
bool Program::operator==(const Program &other) const {
  return this->_name == other._name && this->_command == other._command &&
         ...
         this->_nbprocess == other._nbprocess &&
         this->_autostart == other._autostart &&
         this->_restart == other._restart &&
         this->_exitcodes == other._exitcodes &&
         this->_startdelay == other._startdelay &&
         this->_restartretry == other._restartretry &&
         this->_stopsignal == other._stopsignal &&
         this->_stoptimeout == other._stoptimeout &&
         this->_stdoutfile == other._stdoutfile &&
         this->_stderrfile == other._stderrfile &&
         this->_umask == other._umask && this->_env == other._env;
}
...
std::string Program::getWorkdir() const { return this->_workdir; }
void Program::setNbprocess(int nbprocess) {
  this->_nbprocess = nbprocess;
  this->addProcess();
}
int Program::getNbprocess() const { return this->_nbprocess; }
void Program::setAutostart(bool autostart) { this->_autostart = autostart; }
bool Program::getAutostart() const { return this->_autostart; }
void Program::setRestart(Restart restart) { this->_restart = restart; }
Program::Restart Program::getRestart() const { return this->_restart; }
void Program::setExitcodes(std::vector<int> exitcodes) {
  this->_exitcodes = exitcodes;
}
std::vector<int> Program::getExitcodes() const { return this->_exitcodes; }
void Program::setStartdelay(int startdelay) { this->_startdelay = startdelay; }
int Program::getStartdelay() const { return this->_startdelay; }
void Program::setRestartretry(int restartretry) {
  this->_restartretry = restartretry;
}
int Program::getRestartretry() const { return this->_restartretry; }
void Program::setStopsignal(int stopsignal) { this->_stopsignal = stopsignal; }
void Program::setStopsignalString(const std::string &stopsignal) {
  this->_stopsignal = convertStringToStopsignal(stopsignal);
}
int Program::getStopsignal() const { return this->_stopsignal; }
void Program::setStoptimeout(int stoptimeout) {
  this->_stoptimeout = stoptimeout;
}
int Program::getStoptimeout() const { return this->_stoptimeout; }
...
void Program::setUmask(mode_t umask) { this->_umask = umask; }
mode_t Program::getUmask() const { return this->_umask; }

std::string Program::getStates() {
  std::string states;
  for (const auto &process : this->_processes) {
    states += process.getName() + ": " +
...
void Program::addProcess() {
  if (this->_nbprocess <= 1) {
    Process process(this->_name);
...
Process &Program::getProcess(std::string name) {
  ...
  throw std::runtime_error("Process " + name + " not found in program " +
                           this->_name);
}

void Program::addLog(const Log &log) { this->_logs.push_back(log); }

std::vector<Log> Program::getLogs() const { return this->_logs; }
...
void Program::printProgram() {
  ...
  std::cout << "Program autostart: " << (this->_autostart ? "true" : "false")
            << std::endl;
  std::cout << "Program restart: " << convertRestartToString(this->_restart)
            << std::endl;
  ...
  std::cout << "Program stopsignal: "
            << convertStopsignalToString(this->_stopsignal) << std::endl;
